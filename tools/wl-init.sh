...
#
# tools/wl-init.sh - Provides common functions and help extenders
#
...
validate_tools() {
    ...
    RS_USER=${RS_USER:-rocketskates}
    RS_PASSWORD=${RS_PASSWORD:-r0cketsk8ts}
    export RS_KEY=${RS_KEY:-$RS_USER:$RS_PASSWORD}
}

drpcli() {
    local drpcli_cmd

    drpcli_cmd=$(which drpcli)
    if [[ $drpcli_cmd == "" ]] ; then
        if [[ $(uname -s) == Darwin ]] ; then
            curl -so drpcli https://s3-us-west-2.amazonaws.com/drpcli-bins/darwin/amd64/drpcli
        else
            curl -so drpcli https://s3-us-west-2.amazonaws.com/drpcli-bins/linux/amd64/drpcli
        fi
        chmod +x ./drpcli
    fi
